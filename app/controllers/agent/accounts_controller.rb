class Agent::AccountsController < Agent::BaseController
  ...
  def login
    ...
    self.current_user = User.agent.authenticate(params[:user][:login], params[:user][:password])
    if logged_in?
      successful_login
      render json:{code:0}
    else
  ...
  def logout
    ...
    redirect_to login_agent_accounts_path
  end
