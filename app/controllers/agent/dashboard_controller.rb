...
class Agent::DashboardController <  Agent::BaseController
  ...
  def get_data
    ...
    chongzhi_list=Payment.where(user:User.where(father_id:current_user.id),status: Payment::VERIFYPASS,payment_type: 1,updated_at:[@startTime..@endTime]).group('DATE_FORMAT(payments.updated_at,"%m-%d")').sum(:balance)
    tixian_list=Payment.where(user:User.where(father_id:current_user.id),status: Payment::VERIFYPASS,payment_type: 0,updated_at:[@startTime..@endTime]).group('DATE_FORMAT(payments.updated_at,"%m-%d")').sum(:balance)
